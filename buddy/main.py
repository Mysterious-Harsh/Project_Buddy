...
#             # ── Image path detection (visual cue only) ─────────────────────
#             # The path stays in the message text; the planner routes it to
#             # the vision tool automatically. We just show a brief indicator.
#             try:
#                 _img_paths = extract_image_paths(s)
#                 if _img_paths:
#                     _names = ", ".join(os.path.basename(p) for p in _img_paths)
#                     await ui.ui_output(f"[image detected: {_names}]", kind="buddy")
#             except Exception:
#                 pass
#             # ────────────────────────────────────────────────────────────────
