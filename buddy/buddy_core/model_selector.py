...
_MODEL_BASES: List[_ModelBase] = [
    ...
    _ModelBase(
        ...
        description=(
            "MoE 35B (3B active). Fast inference, high capability. Needs 26 GB+."
        ),
...
def recommend_llm(os_profile: Dict[str, Any]) -> LLMOption:
    ...
    for opt in viable:
        ...
        quant_pref = (
            len(preferred_quants) - preferred_quants.index(opt.quant)
            if opt.quant in preferred_quants
            else 0
        )
...
def viable_options(os_profile: Dict[str, Any]) -> List[LLMOption]:
    ...
    def _sort_key(o: LLMOption):
        ...
        q4_size = next(
            (
                x.size_gb
                for x in QWEN_MODEL_CATALOG
                if x.display_name == o.display_name and x.quant == "Q4_K_M"
            ),
            o.size_gb,
        )
        quant_order = (
            _QUANT_DISPLAY_ORDER.index(o.quant)
            if o.quant in _QUANT_DISPLAY_ORDER
            else len(_QUANT_DISPLAY_ORDER)
        )
    ...
    try:
        ...
        for opt in QWEN_MODEL_CATALOG:
            if opt.filename == saved["filename"]:
                logger.info(
                    "Loaded saved model choice: %s  %s", opt.display_name, opt.quant
                )
                return opt
...
def _print_selection_ui(
    ...
    print(
        _box(
            f"  RAM      {_c_local(str(ram_gb) + ' GB total', 'warn')}"
            f"  ·  {_c_local(str(round(effective_gb, 1)) + ' GB effective', 'ok')}"
        )
    )
    print(_box(f"  Cores    {cores}"))
    if os_ver:
        print(_box(f"  macOS    {os_ver}"))
    print(_box_div())
    print(
        _box(
            "  "
            + _c_local("Choose your LLM:", "tagline")
            + "  "
    ...
    for i, opt in enumerate(options, start=1):
        ...
        if opt.display_name not in seen_models:
            ...
            print(
                _box(
                    f"     {_c_local(opt.display_name, 'key')}"
        ...
        quant_color = (
            "ok"
            if opt.quant in ("Q6_K", "Q8_0")
            else "warn" if opt.quant == "Q5_K_M" else "dim"
        )
        line = (
