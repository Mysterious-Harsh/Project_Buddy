# buddy/ui/stt.py  —  v1.8  (2025-02-24)
"""
...
Proximity gate
──────────────
  When idle (cold), the VAD applies a stricter energy threshold so that
  only nearby, directed speech triggers recording.  Projected voice aimed
  at someone across the room stays below the proximity threshold.

  The instant Buddy confirms speech or returns a transcription the system
  enters conversation mode, dropping back to normal sensitivity for
  conversation_window_sec seconds so follow-up replies are captured
  naturally without needing to speak up.
...
# ── Proximity gate ────────────────────────────────────────────────────────────
#
# HOW THE PROXIMITY GATE WORKS
# ────────────────────────────
# Buddy operates in two sensitivity modes:
#
#   COLD (idle)   start_thr = baseline × proximity_mult   (strict)
#   ACTIVE        start_thr = baseline × speech_trigger_mult  (normal)
#
# In COLD mode the trigger threshold is raised so that only close, directed
# speech (you talking to Buddy from ≤1 m) clears it.  Projected voice aimed
# at someone across the room has lower energy at the microphone due to the
# inverse-square law, so it falls below the proximity threshold and is
# silently ignored.
#
# The moment Buddy confirms speech (on_speech_detected) OR returns a
# transcription, the system enters ACTIVE mode for conversation_window_sec
# seconds.  In ACTIVE mode the threshold drops back to the normal
# speech_trigger_mult value, keeping Buddy naturally responsive during a
# back-and-forth without requiring you to raise your voice on each reply.
#
# After conversation_window_sec of silence the system returns to COLD mode.
#
# TUNING NOTES
# ────────────
# The proximity gate interacts with speech_trigger_mult:
#   proximity_mult should always be > speech_trigger_mult.
#   A ratio of ~1.5–2× is a good starting point.
#   e.g. speech_trigger_mult=3.0, proximity_mult=5.0 → gap of 2×.
#
# If Buddy still responds to side conversations:
#   → RAISE proximity_mult (e.g. 6.0–8.0).
#   → Or RAISE speech_trigger_mult AND proximity_mult together.
#
# If you have to speak too loudly to wake Buddy from cold:
#   → LOWER proximity_mult (e.g. 4.0).
#
# If Buddy stops hearing your follow-up replies mid-conversation:
#   → RAISE conversation_window_sec (e.g. 15.0–20.0).
#   → Or LOWER speech_trigger_mult.
#
# If Buddy stays too sensitive long after you've finished talking:
#   → LOWER conversation_window_sec (e.g. 6.0–8.0).

DEFAULT_PROXIMITY_MULT = 5.0
# Trigger threshold multiplier when idle/cold: start_thr = baseline × this.
# Must be > speech_trigger_mult (default 3.0) to create a proximity gap.
# RAISE (6.0–8.0) → harder to trigger from a distance; requires more directed voice.
# LOWER (3.5–4.0) → easier to trigger; proximity gap shrinks.
# Settable per-instance via proximity_mult= constructor argument.

DEFAULT_CONVERSATION_WINDOW_SEC = 10.0
# Seconds of active-conversation sensitivity after the last speech event.
# After this window expires with no activity, cold proximity mode resumes.
# RAISE (15.0–20.0) → stays responsive longer; good for slow conversations.
# LOWER (5.0–7.0) → snaps back to proximity mode faster after you stop talking.
...
class SpeechToText:
    ...
    Parameters
    ──────────
    whisper_model_size     faster-whisper model name
    ...
    speech_trigger_mult    start_threshold = baseline × this in ACTIVE/conversation mode
                           raise for noisy rooms (default 3.0)
    proximity_mult         start_threshold = baseline × this in COLD/idle mode
                           must be > speech_trigger_mult; raise to ignore far voices
                           (default 5.0)
    conversation_window_sec  seconds to stay in ACTIVE mode after the last speech event
                           (speech detected OR transcription returned); after this the
                           system returns to COLD proximity mode (default 10.0)
    ...
    __slots__ = (
        ...
        "beam_size",
        "speech_trigger_mult",
        "proximity_mult",  # strict threshold mult when cold/idle
        "conversation_window_sec",  # seconds to stay in active mode after last event
        "silence_timeout",
        "calibration_sec",
        "whisper_vad_filter",
        "use_silero_vad",
        ...
        "_conversation_until",  # float — time.time() deadline; 0.0 = cold
        # ── callbacks ───────────────────────────────────────────────────────
        "_cbw",
        "enable_beep",
        "_beep_pcm",
    ...
    def __init__(
        ...
        *,
        ...
        proximity_mult: float = DEFAULT_PROXIMITY_MULT,
        conversation_window_sec: float = DEFAULT_CONVERSATION_WINDOW_SEC,
        ...
        # proximity_mult must be >= speech_trigger_mult; clamp silently if not
        self.proximity_mult = float(max(proximity_mult, speech_trigger_mult))
        self.conversation_window_sec = float(max(0.0, conversation_window_sec))
        ...
        # ── proximity / conversation state ────────────────────────────────────
        # _conversation_until is a plain float timestamp (time.time() deadline).
        # No lock needed: written by stt-tx (after transcription) and by
        # stt-listen (on speech detect); both are single writers on their own
        # paths, and a float write is atomic on CPython (GIL). The VAD loops
        # read it as a local snapshot at the start of each frame — a race
        # between write and read results in at most one frame using the old
        # value, which is harmless.
        self._conversation_until: float = 0.0  # 0.0 = cold/idle
        ...
        if self.debug:
            ...
            logger.info(
                ...
                "mult=%.1f prox=%.1f conv_win=%.1fs "
                "model=%s beam=%d compute=%s vad=%s beep=%s",
                self.sample_rate,
                (
    ...
    def mute(self) -> None:
        ...
        • Reset conversation window → next unmute starts in cold/proximity mode.
        ...
        # Reset conversation state: after a long mute we cannot assume we are
        # still mid-conversation, so start cold when we unmute.
    ...
    # =========================================================================
    # Proximity / conversation window
    # =========================================================================

    @property
    def is_conversation_active(self) -> bool:
        """
        True while the system is in active-conversation mode (normal sensitivity).
        False when cold (proximity-strict mode, higher trigger threshold).
        Read-only; updated automatically by speech detection and transcription.
        """
        return time.time() < self._conversation_until

    def _refresh_conversation_window(self) -> None:
        """
        Extend the active-conversation window by conversation_window_sec from now.

        Called from two places:
          • _on_speech_detected() — the moment VAD confirms real speech
          • _transcribe_worker()  — after Buddy returns a transcription

        Writing a float is atomic under CPython's GIL so no lock is needed.
        The VAD loops read _conversation_until once per frame as a local
        snapshot — a one-frame race is harmless.
        """
        self._conversation_until = time.time() + self.conversation_window_sec
    ...
    def _on_speech_detected(self) -> None:
        ...
        • Refreshes the conversation window so the system stays at normal
          sensitivity for the duration of the exchange.
        • Plays the confirmation beep.
        • Submits the on_interrupt callback.
        All three actions are centralised here so they are always in sync.
        """
    ...
    def _silero_loop(self) -> None:
        ...
        while self._running and not self._muted:
            ...
            # ── Proximity gate: two-mode threshold ────────────────────────────
            # COLD mode  (idle, no recent speech):
            #   start_thr = baseline × proximity_mult  — strict, rejects far voices.
            # ACTIVE mode (within conversation_window_sec of last speech event):
            #   start_thr = baseline × speech_trigger_mult  — normal sensitivity.
            # During recording the threshold is irrelevant (onset already confirmed)
            # so we always use the cheaper speech_trigger_mult there.
            in_conversation = now < self._conversation_until
            trigger_mult = (
                self.speech_trigger_mult
                if (in_conversation or recording)
                else self.proximity_mult
            )
            start_thr = max(adaptive_floor, baseline * trigger_mult)
            ...
            if self.debug and not recording and (now - dbg_last) >= 1.0:
                ...
                logger.info(
                    ...
                    speech_prob,
                    voiced,
                    "active" if in_conversation else "cold",
                )
    ...
    def _custom_loop(self) -> None:
        ...
        while self._running and not self._muted:
            ...
            #   start_thr = baseline × speech_trigger_mult  — normal sensitivity.
            # During recording the threshold is irrelevant (onset already confirmed)
            # so we always use speech_trigger_mult there.
            in_conversation = now < self._conversation_until
            trigger_mult = (
                self.speech_trigger_mult
                if (in_conversation or recording)
            ...
            if self.debug and not recording and (now - dbg_last) >= 1.0:
                ...
                logger.info(
                    ...
                    "crest=%.1f atk=%.1f zcr=%.3f above=%.3f mode=%s",
                    e,
                    baseline,
                    start_thr,
                    crest,
    ...
    def _transcribe_worker(self) -> None:
        ...
        Conversation window
        ───────────────────
        After every non-empty transcription _refresh_conversation_window() is
        called so the proximity gate stays open for a full conversation_window_sec
        after Buddy's last reply.  This means the user's follow-up response is
        always heard at normal sensitivity — they do not need to speak up just
        because the conversation window expired during Whisper's processing time.
        """
        while not self._tx_stop.is_set():
            ...
            if text:
                # Refresh BEFORE submitting the callback so the window is already
                # open by the time the application processes the text and the user
                # begins to reply.
                self._refresh_conversation_window()
...
def live_test() -> None:
    ...
    print("Proximity gate active — speak close to wake, natural distance mid-conv.\n")
