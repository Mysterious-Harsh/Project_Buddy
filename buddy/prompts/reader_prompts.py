...
# ═══════════════════════════════════════════════════════════
# LARGE TEXT READER PROMPT
# ═══════════════════════════════════════════════════════════
#
# Used by Brain.run_reader() → TextReader loop.
# One focused LLM call per paragraph.
#
# Prompt layout (build_prompt):
#   [SYSTEM]  BUDDY_IDENTITY + READER_PROMPT + BUDDY_OUTPUT(schema=READER_SCHEMA)
#   [USER]    _build_context(prior_outputs=rolling_context)
#   [ASST]    "Understood. Ready."
#   [USER]    query + paragraph   ← active task
#   [ASST]    <THINK>             ← prefill
...
>> Does this paragraph contain ANY detail — even the smallest fact, number, name, step, date, or clue — that helps understand or answer the user message? If yes, rewrite it keeping everything. If no, mark as not relevant.
"""
